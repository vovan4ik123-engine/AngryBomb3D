...
uniform bool trajectoryHasHit;
uniform vec3 trajectoryHitPoint;
...
void main()
{
    ...
    float alpha = 1.0f;
    if(trajectoryHasHit &&
       acos(dot(normalize(trajectoryHitPoint - cameraPos), normalize(fragPos - cameraPos))) < 0.1f &&
       distance(cameraPos, fragPos) < distance(cameraPos, trajectoryHitPoint))
        alpha = 0.8f;

    vec3 textureCollor = texture(diffuseTexture, textureCoords).rgb;
    float applyShadow = (diffuse + specular) * shadowMultiplier;
    outColor = vec4((ambientLight + applyShadow) * textureCollor, alpha);
}
